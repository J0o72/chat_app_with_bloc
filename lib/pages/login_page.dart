...
class LoginPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AuthBloc, AuthStatesWithBloc>(
      listener: (context, state) {
        if (state is LoginLoadingWithBloc) {
          isLoading = true;
        } else if (state is LoginSuccessWithBloc) {
          BlocProvider.of<ChatCubit>(context).getMessages();
          Navigator.pushNamed(context, 'chatPage', arguments: email);
          isLoading = false;
        } else if (state is LoginFailureWithBloc) {
          isLoading = false;
                    ...
                    GestureDetector(
                      onTap: () async {
                        if (formKey.currentState!.validate()) {
                          // Login User With AuthCubit
                          // BlocProvider.of<AuthCubit>(context)
                          //     .loginUser(email: email!, password: password!);

                          // Login User With AuthBloc
